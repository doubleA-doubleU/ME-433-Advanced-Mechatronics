...
void APP_Tasks ( void )
{
    ...
    switch ( appData.state )
    {
        ...
        case APP_STATE_SERVICE_TASKS:
        {
            ...
            while (_CP0_GET_COUNT() < 12000) { // wait 0.5 ms -> full on/off cycle = 1 ms or 1 kHz frequency
                while (!PORTBbits.RB4) {
                    LATAbits.LATA4 = 0; // turn off LED while button is pressed
                }
            }
            LATAINV = 0x10; // invert the LED at a frequency of 1 kHz 
            break;
        }
