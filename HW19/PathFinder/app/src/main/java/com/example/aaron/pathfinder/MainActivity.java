...
public class MainActivity extends Activity implements TextureView.SurfaceTextureListener {
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        ...
        parameters.setAutoExposureLock(true); // auto white balance off
    ...
    public void onSurfaceTextureUpdated(SurfaceTexture surface) {
        ...
        if (c != null) {
            ...
            for (int startY = 20; startY <= 460; startY = startY + 20) {// which row in the bitmap to analyze to read
                bmp.getPixels(pixels, 0, bmp.getWidth(), 0, startY, bmp.getWidth(), 1);

                // in the row, see if the pixel is grey
                for (int i = 0; i < bmp.getWidth(); i++) {
                    if (green(pixels[i]) > thresh & green(pixels[i]) < thresh2 &
                            red(pixels[i]) > thresh & red(pixels[i]) < thresh2 &
                            blue(pixels[i]) > thresh & blue(pixels[i]) < thresh2 &
                            (green(pixels[i])- red(pixels[i])) < 10 &
                            (green(pixels[i])- blue(pixels[i])) < 10 &
                            (red(pixels[i])- blue(pixels[i])) < 10) {
